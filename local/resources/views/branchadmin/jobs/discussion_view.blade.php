 ...
 <div class="row">
    <div class="col-xs-12">
      ...
      <div class="top-links btn-group">
         <form class="form-horizontal" role="form" id="testform" method="POST" action="{{ url('/admin/jobs/update_discussion') }}" style="display: none;">
                {!! csrf_field() !!}
                <input type="hidden" name="job_id" value="{{$datas['employee']->jobs_id}}">
                <input type="hidden" name="employee_id[]" value="{{$datas['employee']->id}}">
          </form>

        <a href="{{ url('/admin/jobs/discussion/makepdf/'.$datas['employee']->id) }}" class="btn btn-default btn-xs"><i class="fa fa-file-pdf-o"></i>PDF</a>
        <a href="{{ url('/admin/jobs/discussion/printprofile/'.$datas['employee']->id) }}" target="_blank" class="btn btn-default btn-xs"><i class="fa fa-print"></i>Print</a>
        <a class="btn btn-primary" style="float: right !important;" href="#" onclick="event.preventDefault(); document.getElementById('testform').submit();">Select for Final Interview</a>
