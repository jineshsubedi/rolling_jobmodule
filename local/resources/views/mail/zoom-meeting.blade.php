...
{{--      <img style="width: 100%" src="{{$data['logo']}}">--}}
    ...
    <div style="width: 77%;  background-color: #f9f9f9;  border:1px solid #fff;  border-radius: 3px 3px 0px 0px;  float: right;  padding:18px;  border-bottom: 5px solid #48d617;  box-shadow: 0 10px 6px rgba(119, 119, 199, 0.3);  margin-top:40px;">
      <div style=" width:100%;  float: left;  color: #555555;  font-size: 13px;">
        <h2 style="color: #48d617;  font-weight: 500;  font-size: 18px;">Dear {{$data->firstname}}, </h2>
        <p>We are glad to have you as a new Employee  of Rolling Plans.</p>
        <p>I would like to invite you for an short introduction session schedule for {{$api->start_time}} . The details of the Zoom session ID is mention below:-</p>
        <p>Topic: {{$api->topic}}</p>
        <p>Meeting ID: {{$api->id}}</p>
        <p>Password: {{$api->password}}</p>
        <p>Please click on below button to join zoom meeting or copy this link and pest to your browser <a href="{{$api->join_url}}">{{$api->join_url}}</a></p>
        <p><a href="{{$api->join_url}}" style="padding:7px 15px;border-radius: 3px;color: #fff;box-shadow: 0 3px 2px rgba(119, 119, 199, 0.3);margin-right: 10px;border:1px solid #f25a4d;background-color: #e3271b;">Click Here</a></p>
      </div>
  ...
  <div style="clear: both;"></div>
    <div style="width: 95%; margin:0 auto; position: relative;">
      ...
      <div style="background-color: #00a5e5;color: #fff;font-size:12px;font-weight: 300;text-align: center;padding:2px 0px;margin-top: 5px;float: left;width: 100%;">
        <p>Copyrights © {{date('Y')}}. All Rights Reserved by Rolling Plans</p>
      </div>
  </div>
</body>
</html>